...
const WELL_KNOWN_NS: Record<string, string> = {
  ...
  frbr_core: "http://purl.org/vocab/frbr/core#",
  frbroo: "http://iflastandards.info/ns/fr/frbr/frbroo/",
  abc: "http://metadata.net/harmony/abc#",
...
const expandEntityReferences = (xml: string): string => {
  ...
  const entityDeclRe = /<!ENTITY\s+(\S+)\s+"([^"]+)"\s*>/g;
  let match: RegExpExecArray | null;
  while ((match = entityDeclRe.exec(xml)) !== null) {
    entityMap[match[1]] = match[2];
  }

  // Extract xmlns:prefix="uri" declarations as fallback
  const xmlnsRe = /xmlns:([a-zA-Z_][\w.-]*)="([^"]+)"/g;
  while ((match = xmlnsRe.exec(xml)) !== null) {
    if (!entityMap[match[1]]) {
      entityMap[match[1]] = match[2];
    }
  }

  // Also include well-known and discovered namespaces as fallback
  for (const [prefix, uri] of Object.entries(discoveredNamespaces)) {
    if (!entityMap[prefix]) {
      entityMap[prefix] = uri;
    }
  }
  ...
  // Remove the DOCTYPE declaration (already expanded)
  expanded = expanded.replace(/<!DOCTYPE[^>]*\[[\s\S]*?\]>/g, "");
...
const parseRdfXmlFile = (filePath: string): UnifiedStore => {
  ...
  // Determine base URI from the ontology's own namespace
  const baseUri = discoveredNamespaces.edm || "http://example.org/ontology/";
...
const mergeStores = (stores: UnifiedStore[]): UnifiedStore => {
  ...
  // Collect all quads from all stores, deduplicating by triple key
...
const extractMetadata = (store: UnifiedStore, sourceFiles: string[]): OntologyMetadata => {
  ...
  // Also check for ontology node with empty/relative URI (common in EDM where rdf:about="")
  const allQuads = store.getAllQuads();
  const candidateOntologyUris = new Set<string>();
  for (const q of allQuads) {
    if (q.predicate === `${NS.rdf}type` && q.object === `${NS.owl}Ontology`) {
      candidateOntologyUris.add(q.subject);
    }
  }

  const allOntologyNodes = [...new Set([...ontologyNodes, ...candidateOntologyUris])];

  let title = "";
  let version = "";
  let description = "";

  for (const uri of allOntologyNodes) {
...
const extractExternalVocabularies = (
  ...
  // The edm namespace is the primary one
  if (NS.edm) {
    primaryNamespaces.add(NS.edm);
  }
...
const generateMappingGuide = (
  ...
  const constraints: string[] = [
    ...
    "owl:Restriction constraints (allValuesFrom, someValuesFrom, cardinality) must be respected",
    "EDM follows a ProvidedCHO -> Aggregation -> WebResource pattern for cultural heritage objects",
    "Properties from external vocabularies (DC, DC Terms, ORE, SKOS, FOAF, CRM, FRBR) are valid for use",
    "edm:ProvidedCHO is the central class for representing cultural heritage objects",
    "ore:Aggregation groups related resources (views, metadata) about a cultural heritage object",
  ];
...
const main = async (): Promise<void> => {
  const dataDir = process.env.DATA_DIR || "domain-data/cultural-moma";
  ...
  // Step 3: Merge all stores (deduplicating triples from overlapping files)
