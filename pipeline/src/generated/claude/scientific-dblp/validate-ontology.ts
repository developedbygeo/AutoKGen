...
interface GraphMetadata {
  ...
  validation: { compliant: boolean; errors: number; warnings: number };
}
...
function buildValidLabelsSet(ontology: OntologyStructure): Set<string> {
  ...
  // Also add common external classes used in mapping (foaf:Person, foaf:Organization, skos:Concept, etc.)
  // Derived from ontology namespaces — we include labels for known FOAF/SKOS/FRBR classes
  // since the ontology references them in domain/range
  ...
  // Add "Resource" (common import label)
  labels.add('Resource');
  return labels;
}

function buildPropertyLabelMap(ontology: OntologyStructure): Map<string, string> {
  const map = new Map<string, string>();
  for (const op of ontology.objectProperties) {
    const local = extractLocalName(op.uri);
    map.set(local, op.label);
    map.set(op.label, op.label);
  }
  for (const dp of ontology.dataProperties) {
    const local = extractLocalName(dp.uri);
    map.set(local, dp.label);
    map.set(dp.label, dp.label);
  }
  return map;
}
...
async function validateDataPropertyDomains(
  ...
  for (const dp of ontology.dataProperties) {
    ...
    // Skip if property doesn't exist in graph at all
    if (!existingKeySet.has(propName)) continue;

    const validDomainLabels = new Set(dp.domain.map((d) => resolveClassLabel(d, ontology)));

    // Use sampling: find a small set of nodes with this property that DON'T match domain
    ...
    // Estimate count from label-level checks instead of full scan
...
async function validateDataPropertyDatatypes(
  ...
  for (const dp of ontology.dataProperties) {
    ...
    // Sample nodes with this property (small LIMIT to avoid OOM)
    ...
    if (invalidSamples.length > 0) {
      // Estimate total using sample ratio — avoid full COUNT scan on 16M+ nodes
      const violationRate = invalidSamples.length / sampleRows.length;
      // Use label-specific counts if property is on specific labels
      let estimatedTotal = sampleRows.length; // conservative default
...
async function validateRequiredProperties(
  ...
  for (const pattern of mappingGuide.commonPatterns) {
    ...
    // Check if this label exists in graph
...
async function validateStructure(
  ...
  // Check orphaned nodes — use label-by-label approach to avoid OOM on full scan
  ...
  for (const row of labelNames) {
    ...
    try {
      ...
      // Skip labels that cause OOM — they're likely too large for orphan check
  ...
  // Check that core entity classes have instances
  const coreClasses = graphStats.nodesByType;
  const ontologyClassLabels = ontology.classes.map((c) => toPascalCase(c.label));
  const emptyCore: string[] = [];

  // Check a selection of important classes from the graph stats
  for (const classLabel of ontologyClassLabels.slice(0, 50)) {
    const checkResult = await runCypher<{ count: number }>(
      session,
      `MATCH (n:\`${classLabel}\`) RETURN count(n) AS count LIMIT 1`
    );
    // We don't flag this as a violation — not all ontology classes need instances
  }

  // Verify graph is connected (at least some relationships exist)
  ...
  console.log(`    Found ${formatNumber(totalOrphaned)} orphaned node(s)`);
}
...
async function validatePropertyNamespaces(
  ...
  // Build valid property names from ontology
  const validPropertyNames = new Set<string>();

  for (const dp of ontology.dataProperties) {
    validPropertyNames.add(extractLocalName(dp.uri));
  }
  for (const op of ontology.objectProperties) {
    validPropertyNames.add(extractLocalName(op.uri));
  }

  // Common standard properties that are always valid
  const standardProps = new Set([
    ...
    // Common prefixed property names used in graph
    'pageRange', 'hasPublicationYear', 'hasURL', 'hasISBN',
  ]);

  // Use the existing property keys set (lightweight metadata query already done)
  const invalidProps: Array<{ key: string; count: number }> = [];
  for (const key of existingKeySet) {
    if (validPropertyNames.has(key) || standardProps.has(key)) continue;

    // Check if it matches a known namespace prefix pattern
    const isFromKnownNs = Object.keys(ontology.metadata.namespaces).some((prefix) => {
      return key.startsWith(prefix + '_') || key.startsWith(prefix + ':');
    });
    if (isFromKnownNs) continue;

    invalidProps.push({ key, count: 0 }); // count not available from metadata-only query
  }
  ...
  if (invalidProps.length > 0) {
    violations.push({
      ...
      examples: invalidProps.slice(0, SAMPLE_LIMIT).map((p) => ({
        context: `Property "${p.key}" found on ${formatNumber(p.count)} node(s)`,
      })),
      fixStrategies: [{
...
function calculateComplianceScore(violations: Violation[], totalNodes: number, totalRels: number): number {
  ...
  for (const v of violations) {
    ...
    switch (v.severity) {
      case 'error':
        // Errors: up to 10 points per violation type, scaled by impact
        deductions += Math.min(10, impactRatio * 100 * 2);
        break;
      case 'warning':
        // Warnings: up to 5 points per violation type
        deductions += Math.min(5, impactRatio * 100);
        break;
      case 'info':
        // Info: up to 2 points per violation type
...
function generateFixesCypher(violations: Violation[]): string {
  ...
  const safe: Violation[] = [];
  const moderate: Violation[] = [];
  const destructive: Violation[] = [];

  for (const v of violations) {
    for (const fix of v.fixStrategies) {
      if (fix.risk === 'safe') safe.push(v);
      else if (fix.risk === 'moderate') moderate.push(v);
      else destructive.push(v);
    }
  }
  ...
  if (safe.length > 0) {
    ...
    lines.push('');
    for (const v of safe) {
      for (const fix of v.fixStrategies.filter((f) => f.risk === 'safe')) {
        lines.push(`// ${fix.description}`);
        lines.push(`// Violation: ${v.description}`);
  ...
  if (moderate.length > 0) {
    ...
    for (const v of moderate) {
      for (const fix of v.fixStrategies.filter((f) => f.risk === 'moderate')) {
        lines.push(`// ${fix.description}`);
        lines.push(`// Violation: ${v.description}`);
  ...
  if (destructive.length > 0) {
    ...
    for (const v of destructive) {
      for (const fix of v.fixStrategies.filter((f) => f.risk === 'destructive')) {
        lines.push(`// ${fix.description}`);
        lines.push(`// Violation: ${v.description}`);
...
async function main(): Promise<void> {
  ...
  // Load graph statistics from graph-data.json metadata (just the first few KB)
  let graphStats: GraphStatistics = { totalNodes: 0, nodesByType: {}, totalRelationships: 0, relationshipsByType: {} };
  if (fs.existsSync(GRAPH_FILE)) {
    // Read last portion of file for statistics
  ...
  try {
    // Fetch property keys once (lightweight metadata query)
