...
function loadSupplementaryIndex(): SupplementaryFileEntry[] | null {
...
function classifyValue(value: string): keyof TypeAccumulator {
...
function resolveType(acc: TypeAccumulator, total: number): ColumnProfile["inferredType"] {
...
function createColumnAccumulator(name: string): ColumnAccumulator {
  return {
    name,
    nonEmpty: 0,
    missing: 0,
...
function printSummary(profile: DatasetProfile): void {
...
async function main(): Promise<void> {
  console.log("\n=== Data Profiler (Scientific-DBLP — Streaming) ===\n");
  ...
  // Load supplementary files index if available
