...
async function createConstraintsAndIndexes(
  ...
  // Extract unique labels from nodesByType keys — use URI fragment (after last / or #)
  const labels = new Set<string>();
  for (const typeKey of Object.keys(nodesByType)) {
    const label = extractLocalName(typeKey);
  ...
  // Create indexes on frequently queried properties for EDM types
  const indexableProps: Record<string, string[]> = {
    ProvidedCHO: ['Title', 'objectId'],
    Agent: ['prefLabel_DisplayName'],
    Place: ['name'],
    TimeSpan: ['label'],
    Concept: ['prefLabel'],
    WebResource: ['resourceUrl'],
  };
...
async function importRelationships(
  ...
  console.log(`  Array starts at byte offset ${arrayStartOffset} (${(arrayStartOffset / 1024 / 1024).toFixed(1)} MB)\n`);
  ...
  for await (const line of rl) {
    ...
    // Flush any full batches every 2500 parsed lines
...
async function validateImport(
  ...
  // Count nodes by label (exclude Resource meta-label)
  const nodeResults = await runQuery(driver, `
    MATCH (n)
    WITH n, [l IN labels(n) WHERE l <> 'Resource'][0] AS label
  ...
  // Check for nodes missing id property
  const missingIdResult = await runQuery(driver, 'MATCH (n) WHERE n.id IS NULL RETURN labels(n) AS labels, count(*) AS count') as any[];
  if (missingIdResult.length > 0) {
    console.log('  ⚠ Nodes missing id property:');
    for (const row of missingIdResult) {
      const count = typeof row.count === 'object' && 'toNumber' in row.count ? row.count.toNumber() : Number(row.count);
      console.log(`    ${row.labels}: ${formatNumber(count)}`);
    }
    console.log('');
  }
...
async function main(): Promise<void> {
  ...
  console.log('╔═════════════════════════════════════════════════════════╗');
  console.log('║    Neo4j Import — Cultural MoMA Knowledge Graph (EDM)   ║');
  console.log('╚═════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    console.log(`║  Nodes:          ${formatNumber(validation.actualNodes).padEnd(38)}║`);
    console.log(`║  Relationships:  ${formatNumber(validation.actualRels).padEnd(38)}║`);
    ...
    console.log(`║  Orphaned:       ${formatNumber(validation.orphanedNodes).padEnd(38)}║`);
