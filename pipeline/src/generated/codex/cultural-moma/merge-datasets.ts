...
const JOIN_KEY_PATTERNS = [/^id$/i, /id$/i, /_id$/i, /identifier$/i, /code$/i];
...
function inferFormat(extension: string, sample: string): string {
  if (extension === ".json" && /^\s*\{[\s\S]*\}\s*$/.test(sample) === false && /^\s*\[/.test(sample) === false) {
...
function looksLikeHeader(line: string, delimiter: string): boolean {
  const values = line.split(delimiter).map((part) => part.trim()).filter((part) => part !== "");
...
async function indexSupplementaryFiles(): Promise<SupplementaryFileInfo[]> {
  ...
  for (const file of files) {
    ...
    const stat = fs.statSync(fullPath);
    const sample = readSample(fullPath);
    const lines = sample.split(/\r?\n/).map((line) => stripBom(line)).filter((line) => line.trim() !== "");
    ...
    if (["csv", "tsv", "txt"].includes(format)) {
      ...
      rowCount = format === "jsonl" ? await countJsonlRows(fullPath) : 1;
    }
...
function findSchemaForHeaderlessTxt(
  ...
  const match = supplementaryFiles.find((entry) => {
    ...
    const sameWidth = entry.columns.length === fieldCount;
    return sameWidth && sameBase;
  });
...
async function detectXmlRecordTag(filePath: string, dtdElements: string[]): Promise<{ rootTag?: string; recordTag?: string }> {
  return new Promise((resolve, reject) => {
    ...
    const finish = () => {
      ...
      const dtdPreferred = ordered.find(([tag]) => dtdElements.map((value) => value.toLowerCase()).includes(tag));
    ...
    parser.on("opentag", (node: sax.Tag) => {
      ...
      if (!insideRecord && tag === schema.recordTag) {
        ...
        for (const [attr, value] of Object.entries(node.attributes)) {
          ...
          setValue(currentRow, key, String(value));
        }
        return;
      }
...
async function main(): Promise<void> {
  console.log("=== Cultural-MoMA Data Merger ===");
  ensureDirectories();
